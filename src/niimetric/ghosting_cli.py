...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="Add random ghosting artifacts to a NIfTI image using TorchIO.")
    ...
    parser.add_argument("-s", "--severity", type=int, choices=[1, 2, 3, 4, 5], default=3, help="Severity of ghosting (1-5). Default is 3.")
...
def main(args=None):
    ...
    severity_map = {
        1: (0.4, 0.5), # intensity bounds
        2: (0.5, 0.6),
        3: (0.6, 0.7),
        4: (0.7, 0.8),
        5: (0.8, 0.9)
    }
    intensity = severity_map[parsed.severity]
    ...
    print(f"Applying TorchIO RandomGhosting (severity={parsed.severity}, intensity={intensity})...")
    # TorchIO requires (C, W, H, D)
    tensor = img_norm[np.newaxis, ...]
    subject = tio.Subject(t1=tio.ScalarImage(tensor=tensor, affine=img.affine))
    
    # num_ghosts is usually default between [4, 10], we can just use default ranges for components but map severity to intensity.
    transform = tio.RandomGhosting(intensity=intensity)
